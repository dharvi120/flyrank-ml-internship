...
# --- ARCHITECTURAL FIX: Dynamic Pathing ---
original_scripts_dir = Path(__file__).resolve().parents[2] / "scripts"
sys.path.append(str(original_scripts_dir))
# ------------------------------------------
...
def main() -> None:
    ...
    # --- ARCHITECTURAL FIX: Dynamic Column Selection ---
    prediction_cols = ["content_id", "best_model_name", "best_model_probability"]
    prob_cols = [col for col in prediction_frame.columns if col.startswith("prob_")]
    selected_cols = list(set(prediction_cols + prob_cols))
    
    final_frame = baseline_frame.merge(
        prediction_frame[selected_cols],
        on="content_id",
        how="left",
    )
    # ---------------------------------------------------
    ...
    # --- ARCHITECTURAL FIX: Vectorized Confidence Scoring ---
    # Replaced slow .apply(axis=1) with lightning-fast boolean masking
    final_frame["confidence"] = "low"
    final_frame.loc[final_frame["final_refresh_score"] >= medium_threshold, "confidence"] = "medium"
    
    high_mask = (
        (final_frame["final_refresh_score"] >= high_threshold) &
        (final_frame["impressions_90d"] >= 500) &
        (final_frame["sessions_90d"] >= 10) &
        (final_frame["best_model_probability"] >= 0.5)
    )
    final_frame.loc[high_mask, "confidence"] = "high"
    # --------------------------------------------------------

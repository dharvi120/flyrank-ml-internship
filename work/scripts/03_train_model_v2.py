...
# --- ARCHITECTURAL FIX ---
# Dynamically add the original scripts directory to Python's path
# so we don't have to violate the "don't copy unmodified files" rule.
...
# ... (keep the rest of your imports below this, including ml_utils)
import argparse
from pathlib import Path
...
def build_feature_matrix(frame: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    ...
    # --- ARCHITECTURAL FIX: XGBoost feature name sanitization ---
    # Replace any [, ], or < characters with an underscore so XGBoost doesn't crash.
    feature_frame.columns = feature_frame.columns.str.replace(r'[\[\]<]', '_', regex=True)
    # ------------------------------------------------------------
